...
/*LONG WINAPI StreamingFunc(LPVOID lParam) //change by guan
{
...
void CUDASceneRepChunkGrid::streamOutToCPUPass0GPU(const vec3f& posCamera, float radius, bool useParts, bool multiThreaded /*= true*/ )
{
	...
	if (multiThreaded) {
		...
		//ReleaseMutex(hMutexOut);change by guan
	}
}
...
{
	if (multiThreaded) {
		//WaitForSingleObject(hEventInProduce, INFINITE);//change by guan
...
void CUDASceneRepChunkGrid::debugCheckForDuplicates() const
{
	...
	std::cout << __FUNCTION__ <<" : OK!" << std::endl;//add << after "__FUNCTION__" change by guan
}
