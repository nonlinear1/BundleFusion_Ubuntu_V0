...
template<> void CUDAImageUtil::copy<float>(float* d_output, float* d_input, unsigned int width, unsigned int height){MLIB_CUDA_SAFE_CALL(cudaMemcpy(d_output, d_input, sizeof(float)*width*height, cudaMemcpyDeviceToDevice));}//change by guan
template<> void CUDAImageUtil::copy<uchar4>(uchar4* d_output, uchar4* d_input, unsigned int width, unsigned int height){MLIB_CUDA_SAFE_CALL(cudaMemcpy(d_output, d_input, sizeof(uchar4)*width*height, cudaMemcpyDeviceToDevice));}//change by guan
