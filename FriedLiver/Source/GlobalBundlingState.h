...
#define X_GLOBAL_BUNDLING_APP_STATE_FIELDS \
	...
	X(unsigned, s_maxNumImages) \
	X(unsigned, s_submapSize) \
	X(unsigned, s_widthSIFT) \
	X(unsigned, s_heightSIFT) \
	X(unsigned, s_maxNumKeysPerImage) \
	X(unsigned, s_numLocalNonLinIterations) \
	X(unsigned, s_numLocalLinIterations) \
	X(unsigned, s_numGlobalNonLinIterations) \
	X(unsigned, s_numGlobalLinIterations) \
	X(unsigned, s_downsampledWidth) \
	X(unsigned, s_downsampledHeight) \
	...
	X(unsigned, s_minNumMatchesLocal) \
	X(unsigned, s_minNumMatchesGlobal) \
	...
	X(unsigned, s_numOptPerResidualRemoval) \
	...
	X(unsigned, s_denseOverlapCheckSubsampleFactor)
